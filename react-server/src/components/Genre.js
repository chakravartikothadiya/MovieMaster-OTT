...
function Genre() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const requestToGetGenre = await axios.get(
      ...
      if (genreName != undefined) {
        let recommend = await axios.get(
          `http://localhost:8000/recommend/genres/${genreName}/6`
        );
  ...
  return (
    <div>
      <HomeBanner
        fetchURL={`https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&with_genres=${id}`}
      />
